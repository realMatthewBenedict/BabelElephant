...
def read_audio_files(index_path: Path, directory: Path) -> list[FileResult]:
    ...
    for index, row in df.iterrows():
        ...
        if file_name in result:
            sample, frame = convert_time(row, result[file_name].audio_tensor, result[file_name].sample_rate, frame_step)
...
def reconstruct_audio_tensor(audio_orig: tf.Tensor, new_spectrogram: tf.Tensor) -> tf.Tensor:
    # Original audio
    global frame_length, frame_step, fft_length
    stft_original = tf.signal.stft(audio_orig, frame_length=frame_length, frame_step=frame_step, fft_length=fft_length)
    ...
    audio_reconstruct = tf.signal.inverse_stft(
        stft_modified, frame_length=frame_length, frame_step=frame_step, fft_length=fft_length
    )
